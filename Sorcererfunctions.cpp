...
#include "Paladinfunctions.cpp"

using namespace std;

Sorcerer::Sorcerer(string N,int L, int H_P, int M_P, int S,int D, int A, int M, int E): Hero(N,L,H_P,M_P,S,D,A,M,E){
		dexterity=dexterity+8;// sorcerer has more dexterity , agility than  warrior and paladin
		agility=agility+10;
		cout<<"a sorcerer has been created"<<endl;
}
	
Sorcerer::~Sorcerer(){
		cout<<"a sorcerer has been deleted"<<endl;
}

void Sorcerer::LevelUp(){					// when the sorcerers level up , gain some extra dexterity and agility
	Hero::LevelUp();
	dexterity=dexterity+8;
	agility=agility+10;
}

int Sorcerer::checkInventory(){  // it shows the items / spells in the market
...
bool Sorcerer::BuySpell(Spell& it){
...
vector <Item*>:: iterator Sorcerer::sellItem(int a){
...
vector <Spell*>:: iterator Sorcerer::sellSpell(int a){
...
int Sorcerer::getsize(){
	return listofItems.size();
}

void Sorcerer::delete_Item(vector <Item*>:: iterator it){
	listofItems.erase(it);
}

void Sorcerer::delete_Spell(vector <Spell*>:: iterator it){
	listofSpells.erase(it);
}

void Sorcerer::displayStats(){
	cout<<"Sorcerer with name: "<< name<< " is in level: "<< level<<", his healthPower is: "<< healthPower;
...
void Sorcerer::equip(int number){
	int i=1, ch;
	vector<Item*>::iterator it;
...
int Sorcerer::attack(int number,vector<Monster*> *FightMonster){
...
int Sorcerer::castSpell( vector<Monster*> FightMonster, int round_now ){
	int i =1 , ch , rounds = 0;
...
bool Sorcerer::get_armor(){
	return armor;
}

int Sorcerer::get_magicPower(){
	return magicPower;	
}

void Sorcerer::increase_magicPower(){
	magicPower=magicPower+ (magicPower)/4;
	if(magicPower>50)
		magicPower=50;
}

int Sorcerer::get_healthPower(){
	return healthPower;
}

void Sorcerer::increase_healthPower(bool alive){
	if(alive==true){
...
void Sorcerer::decrease_healthPower(int i){
...
int Sorcerer::get_dexterity(){
	return dexterity;
}

int Sorcerer::get_agility(){
	return agility;
}

void Sorcerer::set_magicPower(int a){
	magicPower = a;
}
