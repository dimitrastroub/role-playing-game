...
#include <string>
#include <cstring>
#include "Sorcererfunctions.cpp"

using namespace std;

Warrior::Warrior(string N,int L, int H_P, int M_P, int S,int D, int A, int M, int E): Hero(N,L,H_P,M_P,S,D,A,M,E){
		strength=strength+15; 										// warrior has more strength , agility than sorcerer and paladin
		agility=agility+10;
		cout<<"a warrior has been created"<<endl;
}

Warrior::~Warrior(){
		cout<<"a warrior has been deleted"<<endl;
}

void Warrior::LevelUp(){											// when the warriors level up , gain some extra strength and agility
	Hero::LevelUp();
...
void Warrior::displayStats(){
	cout<<"Warrior with name: "<< name<< " is in level: "<< level<<", his healthPower is: "<< healthPower;
	cout<<" his strength is: "<< strength<<", his dexterity is: "<< dexterity<< " and his agility is: "<<agility<< endl;
}

bool Warrior::BuyItem(Item& it){
...
bool Warrior::BuySpell(Spell& it){
...
vector <Item*>:: iterator Warrior::sellItem(int a){
	vector<Item*>::iterator it,at;
	int count=0,price;
	for(it=listofItems.begin(); it != listofItems.end(); ++it){
		cout<<"count="<<count<<endl;
		if(count==a-1){ // when we found the item we want to sell
			price=(*it)->get_price(); 
...
vector <Spell*>:: iterator Warrior::sellSpell(int a){
...
void Warrior::delete_Item(vector <Item*>:: iterator it){
	listofItems.erase(it);
}


void Warrior::delete_Spell(vector <Spell*>:: iterator it){
	listofSpells.erase(it);
}

int Warrior::checkInventory(){ // it shows the items / spells in the market
...
int Warrior::attack(int number,vector<Monster*> *FightMonster){
...
int Warrior::castSpell( vector<Monster*> FightMonster, int round_now ){
	int i =1 , ch , rounds = 0;
...
bool Warrior::get_armor(){
	return armor;
}

int Warrior::get_magicPower(){
	return magicPower;	
}

void Warrior::increase_magicPower(){
	magicPower=magicPower+ (magicPower)/4;
	if(magicPower>50)
		magicPower=50;
}

int Warrior::get_healthPower(){
	return healthPower;
}

void Warrior::increase_healthPower(bool alive){
	if(alive==true){
...
void Warrior::decrease_healthPower(int i){
...
int Warrior::get_dexterity(){
	return dexterity;
}

int Warrior::get_agility(){
	return agility;
}

void Warrior::set_magicPower(int a){
	magicPower = a;
}
